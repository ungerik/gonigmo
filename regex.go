...
func NewRegexp(pattern string, option int) (re *Regexp, err error) {
	...
	if errCode != C.ONIG_NORMAL {
		...
		re.subexpNames = make(map[int]string, len(re.namedGroupInfo))
		for name, i := range re.namedGroupInfo {
			re.subexpNames[i] = name
		}
		//eventually not works, we need do it manually
...
// in this Regexp.
func (re *Regexp) SubexpNames() map[int]string {
...
func (re *Regexp) expand(dst []byte, template string, bsrc []byte, src string, match []int) []byte {
	for len(template) > 0 {
		...
		name, num, rest, ok := extractTemplate(template)
		if !ok {
...
// extractTemplate returns the name from a leading "$name" or "${name}" in str.
// If it is a number, extract returns num set to that number; otherwise num = -1.
func extractTemplate(str string) (name string, num int, rest string, ok bool) {
	if len(str) < 2 || str[0] != '$' {
		return
	}
	brace := false
